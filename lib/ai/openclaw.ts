...
async function analizarConLLM(usuario: string, accion: string, modulo: string, detalleRaw: string) {
  ...
  if (payload && payload.changes) {
    ...
    if (keys.length > 0) {
      ...
      analisisInteligente = `El usuario ${usuario} realizó la acción ${accion} en el módulo ${modulo}. Contexto: ${contextMsg}. Cambios exactos detectados -> ${diffText}`;
    } else {
      analisisInteligente = `El usuario ${usuario} realizó la acción ${accion} en el módulo ${modulo}. Contexto: ${contextMsg}. (No se detectaron cambios en los campos).`;
    }
  } 
  // 3. Casos anteriores y reglas de negocio genéricas
  else if (accion === 'LOGIN' && detalleRaw.includes('nocturno')) {
    ...
    analisisInteligente = `✏️ MODIFICACIÓN: El usuario ${usuario} alteró un registro de Caja Chica existente. Razón aparente: ${detalleRaw || 'No especificada'}. Se recomienda revisar el comprobante adjunto para asegurar que el cambio es válido.`;
    ...
    analisisInteligente = `🗑️ ELIMINACIÓN CRÍTICA: El usuario ${usuario} borró información del módulo ${modulo}. Detalle de lo borrado: ${payload?.message || detalleRaw}.`;
  } else {
    // Respuesta genérica inteligente
    const textMsg = payload?.message || detalleRaw;
    analisisInteligente = `El usuario ${usuario} realizó la acción ${accion} en el módulo ${modulo}. Contexto: ${textMsg}`;
  }
