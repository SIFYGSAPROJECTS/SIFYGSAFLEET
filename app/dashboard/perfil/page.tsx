      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl h-fit">
          <div className="flex flex-col items-center text-center">
            ...
            <span className="mt-6 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest border bg-[#6366F1]/10 text-[#6366F1] border-[#6366F1]/30">
        ...
        <div className="md:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-xl border-t-4 border-t-[#6366F1]">
          ...
          <form onSubmit={actualizarMiPassword} className="flex flex-col gap-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="relative">
                  <input type={mostrarNueva ? "text" : "password"} placeholder="Mínimo 6 caracteres" className="w-full bg-slate-950 border border-slate-700 rounded-lg pl-4 pr-12 py-3 text-white focus:border-[#6366F1] outline-none transition-all font-mono" value={nuevaPassword} onChange={(e) => setNuevaPassword(e.target.value)} />
                  <button type="button" onClick={() => setMostrarNueva(!mostrarNueva)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-[#6366F1] transition-colors">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="relative">
                  <input type={mostrarConfirmar ? "text" : "password"} placeholder="Repite tu contraseña" className="w-full bg-slate-950 border border-slate-700 rounded-lg pl-4 pr-12 py-3 text-white focus:border-[#6366F1] outline-none transition-all font-mono" value={confirmarPassword} onChange={(e) => setConfirmarPassword(e.target.value)} />
                  <button type="button" onClick={() => setMostrarConfirmar(!mostrarConfirmar)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-[#6366F1] transition-colors">
            ...
            <button type="submit" disabled={guardando || !nuevaPassword || !confirmarPassword} className="bg-[#6366F1] hover:bg-[#4F46E5] disabled:bg-slate-800 disabled:text-slate-500 text-white px-6 py-4 rounded-lg flex items-center justify-center gap-2 font-bold transition-all shadow-lg mt-2">
