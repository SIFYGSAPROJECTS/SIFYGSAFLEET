...
export default function SeguridadPage() {
  ...
  return (
    <div className="w-full">
      {/* BUSCADOR RESPONSIVE */}
      <div className="bg-[var(--bg-floating)] p-5 sm:p-8 rounded-2xl border border-[var(--border-cream)] shadow-xl mb-8 border-t-4 border-t-amber-600">
        <label className="block text-base sm:text-lg font-medium text-[var(--text-muted)] mb-4 text-center font-serif">
        ...
        <div className="flex flex-col sm:flex-row max-w-2xl mx-auto gap-3">
          <input 
            ...
            className="w-full bg-white border-2 border-[var(--border-cream)] rounded-xl px-4 py-3 text-[var(--text-main)] focus:border-[#71717a] outline-none transition-all font-medium placeholder:text-stone-400 text-sm sm:text-base"
            value={busqueda}
          ...
          <button 
            ...
            disabled={cargando}
            className="w-full sm:w-auto bg-[#71717a] hover:bg-[#52525b] disabled:bg-[var(--bg-floating)] text-white px-8 py-3 rounded-xl flex items-center justify-center gap-2 font-bold transition-all shadow-lg active:scale-95"
          >
      ...
      {empleadoEncontrado ? (
        <div className="bg-[var(--bg-floating)] border border-[var(--border-cream)] rounded-2xl p-5 sm:p-8 shadow-xl animate-in slide-in-from-bottom-4 duration-500">
          <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-8">
            
            <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left w-full">
              <div className="bg-[var(--bg-floating)] p-4 rounded-2xl border border-[var(--border-cream)] shadow-inner shrink-0">
                <User className="w-10 h-10 text-zinc-500" />
              </div>
              <div className="overflow-hidden w-full">
                <h2 className="text-xl sm:text-2xl font-black text-[var(--text-main)] truncate font-serif">
                  {empleadoEncontrado.Nombre_Empleado} {empleadoEncontrado.A_Paterno}
                </h2>
                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 mt-1 text-[var(--text-muted)] text-xs sm:text-sm font-mono">
            ...
            <div className="flex flex-col gap-4 w-full lg:w-auto lg:min-w-[350px] bg-[var(--bg-floating)]/50 p-4 sm:p-6 rounded-2xl border border-[var(--border-cream)]">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 bg-white border border-[var(--border-cream)] rounded-xl px-4 py-3.5 text-[var(--text-main)] focus:border-zinc-500 outline-none transition-all text-sm font-mono"
                  value={nuevaPassword}
                  onChange={(e) => setNuevaPassword(e.target.value)}
                />
                <button 
                  onClick={generarPassword}
                  className="bg-[var(--bg-floating)] hover:bg-[#71717a] text-[var(--text-muted)] hover:text-white px-4 py-3.5 rounded-xl flex items-center justify-center transition-all shadow-lg active:scale-90"
              ...
              <button 
                ...
                className="w-full bg-zinc-500 hover:bg-zinc-600 disabled:bg-[var(--bg-floating)] disabled:text-[var(--text-muted)] text-white py-4 rounded-xl flex items-center justify-center gap-3 font-bold transition-all shadow-lg active:scale-95"
              >
        ...
        !cargando && !mensaje.texto && (
          <div className="bg-[var(--bg-floating)]/40 border-2 border-dashed border-[var(--border-cream)] rounded-2xl p-10 sm:p-20 text-center">
            <Lock className="w-10 h-10 sm:w-12 h-12 text-slate-700 mx-auto mb-4 opacity-30" />
            <p className="text-[var(--text-muted)] font-bold uppercase tracking-[0.2em] text-[10px] sm:text-xs">
