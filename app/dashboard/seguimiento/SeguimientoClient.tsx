...
const TicketCard = memo(({ ticket, isAdmin, infoMemoria, setDatosTemporales }: any) => {
  ...
  return (
    <div id={`ticket-${ticket.Pk_folio_ticket}`} className={`bg-[var(--bg-floating)] rounded-xl shadow-xl border-x border-b border-[var(--border-cream)] border-t-4 p-5 sm:p-8 transition-colors ${esFinalizadoVisual ? 'border-t-emerald-500' : 'border-t-[#71717a]'}`}>
        ...
        {isAdmin && !esFinalizadoReal && (
          <div className="bg-[var(--bg-floating)] p-4 rounded-xl border border-[var(--border-cream)] shadow-inner w-full md:w-auto animate-in fade-in">
            <StatusUpdater 
              ...
              onUpdateTemporal={(info: any) => setDatosTemporales((prev: any) => ({ ...prev, [ticket.Pk_folio_ticket]: info }))}
            />
          </div>
        )}
        {isAdmin && esFinalizadoReal && (
  ...
  return (
    <div className="space-y-6">
      ...
      {ticketsFiltrados.map((ticket: any) => {
        ...
        return (
          <TicketCard
            key={ticket.Pk_folio_ticket}
            ticket={ticket}
            isAdmin={isAdmin}
            infoMemoria={infoMemoria}
            setDatosTemporales={setDatosTemporales}
          />
        );
      })}
    </div>
