    ...
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 mb-6 border-b border-slate-800 pb-4">
        <div className="flex gap-3 overflow-x-auto scrollbar-hide">
          <button onClick={() => setFiltroTab('Activo')} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm sm:text-base whitespace-nowrap transition-all ${filtroTab === 'Activo' ? 'bg-[#FF7420]/10 text-[#FF7420] border border-[#FF7420]/50 shadow-[0_0_15px_rgba(255,116,32,0.15)]' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-900'}`}>
            <ShieldCheck size={18} /> Personal Activo
            <span className={`ml-1 px-2 py-0.5 rounded-full text-[10px] ${filtroTab === 'Activo' ? 'bg-[#FF7420] text-white' : 'bg-slate-800 text-slate-400'}`}>{totalActivos}</span>
          </button>
        ...
        <button onClick={abrirModalNuevo} className="w-full sm:w-auto bg-[#FF7420] hover:bg-[#E6681C] text-white px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95">
          ...
          <div className="bg-slate-900 w-full max-w-2xl rounded-t-3xl sm:rounded-xl shadow-2xl border-t sm:border border-slate-800 overflow-hidden flex flex-col max-h-[95vh]">
            ...
            <form onSubmit={guardarEmpleado} className="p-6 overflow-y-auto pb-10 sm:pb-6 space-y-6">
                ...
                <div className="sm:col-span-2 space-y-2 relative">
                  ...
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <Car size={16} className={busquedaVehiculo ? "text-[#FF7420]" : "text-slate-500"} />
                    </div>
                    <input 
                      type="text"
                      ...
                      className="w-full pl-10 pr-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white focus:ring-2 focus:ring-[#FF7420] outline-none transition-all placeholder:text-slate-700 text-sm"
                    />
                        ...
                        {sugerenciasVehiculos.length > 0 ? (
                          sugerenciasVehiculos.map((unidad) => {
                            ...
                            return (
                              ...
                              >
                                <div>
                                  <p className={`text-sm font-bold transition-colors ${ocupadoPorOtro ? 'text-slate-400 group-hover:text-red-400' : 'text-white group-hover:text-[#FF7420]'}`}>
                                ...
                                <PlusCircle size={16} className={`transition-colors ${ocupadoPorOtro ? 'text-slate-600 group-hover:text-red-400' : 'text-slate-600 group-hover:text-[#FF7420]'}`} />
                              </button>
                            );
                          })
                        ) : (
              ...
              <div className="pt-6 border-t border-slate-800 flex flex-col-reverse sm:flex-row justify-end gap-3 mt-4">
                ...
                <button type="submit" disabled={guardando} className="w-full sm:w-auto flex justify-center items-center gap-2 px-8 py-3.5 sm:py-2.5 text-white font-black rounded-xl bg-[#FF7420] hover:bg-[#E6681C] shadow-lg disabled:opacity-50 transition-all uppercase tracking-wider text-xs sm:text-sm">
