...
import { UserPlus, X, Pencil, ShieldAlert, ShieldCheck, UserMinus, UserCheck, Loader2, AlertTriangle, Laptop, PlusCircle, Download, ArrowLeft } from 'lucide-react';
...
export default function PersonalPage() {
  ...
  const [equipos, setEquipos] = useState<Computo[]>([]); // Estado para la flota TI
  ...
  //  CARGAMOS LOS EQUIPOS DE CÓMPUTO DE FORMA SEGURA APUNTANDO A /api/computo/inventario
  ...
  useEffect(() => {
    const updateHeaderHeight = () => {
      const header = document.getElementById('sticky-header-computo-empleados');
      if (header) {
  ...
  useEffect(() => {
    cargarEmpleados();
    cargarEquipos(); // Traemos los equipos al abrir la página
  }, []);
  ...
  const guardarEmpleado = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        cargarEmpleados();
        cargarEquipos(); // Recargamos para refrescar quién tiene qué equipo
        if (!modoEdicion) {
  ...
  return (
    <div className="min-h-screen bg-transparent relative">
      <div className="pt-2 pb-8 sm:pt-4 sm:pb-8 max-w-[95%] mx-auto relative">
        ...
        <div id="sticky-header-computo-empleados" className={`sticky top-[72px] z-40 transition-all duration-300 pt-2 pb-2 mb-4 px-0 ${scrolled ? 'bg-[#f8fafc] border-b border-[var(--border-cream)] shadow-xl' : 'bg-transparent border-transparent'}`}>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[var(--bg-screen)] border-b border-[var(--border-cream)] text-[var(--text-muted)] text-sm uppercase tracking-wider">
                  <th className="sticky z-30 p-4 font-semibold border-b-2 border-stone-400/20 bg-[var(--bg-screen)] shadow-sm rounded-tl-lg" style={{ top: `${headerHeight}px` }}>Empleado</th>
                  <th className="sticky z-30 p-4 font-semibold border-b-2 border-stone-400/20 bg-[var(--bg-screen)] shadow-sm" style={{ top: `${headerHeight}px` }}>Contacto</th>
                  <th className="sticky z-30 p-4 font-semibold border-b-2 border-stone-400/20 bg-[var(--bg-screen)] shadow-sm" style={{ top: `${headerHeight}px` }}>Puesto</th>
                  <th className="sticky z-30 p-4 font-semibold border-b-2 border-stone-400/20 bg-[var(--bg-screen)] shadow-sm" style={{ top: `${headerHeight}px` }}>Equipo Asignado</th>
                  <th className="sticky z-30 p-4 font-semibold border-b-2 border-stone-400/20 bg-[var(--bg-screen)] shadow-sm text-center" style={{ top: `${headerHeight}px` }}>Nivel</th>
                  <th className="sticky z-30 p-4 font-semibold border-b-2 border-stone-400/20 bg-[var(--bg-screen)] shadow-sm text-center rounded-tr-lg" style={{ top: `${headerHeight}px` }}>Acciones</th>
                </tr>
              </thead>
              <tbody className="">
                {cargando ? (
                  <tr><td colSpan={6} className="text-center p-8 text-[var(--text-muted)]">Cargando personal... 👥</td></tr>
                ) : empleadosFiltrados.length === 0 ? (
                  <tr><td colSpan={6} className="text-center p-8 text-[var(--text-muted)] uppercase font-bold tracking-widest">No hay usuarios en esta lista</td></tr>
                ) : (
                  empleadosFiltrados.map((emp) => (
                    <tr key={emp.Email} className="hover:bg-[var(--bg-hover)] even:bg-[var(--bg-screen)] transition-colors">
                      ...
                      <td className="p-4 text-sm text-[var(--text-muted)]">
                        {equipos.find(eq => eq.Email_Empleado === emp.Email) ? (
                          <div>
                            <div className="font-bold text-[var(--text-main)]">
                              {equipos.find(eq => eq.Email_Empleado === emp.Email)?.C_Interno}
                            </div>
                            <div className="text-[10px] text-[var(--text-muted)] uppercase">
                              {equipos.find(eq => eq.Email_Empleado === emp.Email)?.Marca} {equipos.find(eq => eq.Email_Empleado === emp.Email)?.Modelo}
                            </div>
                          </div>
                        ) : (
                          <span className="text-stone-400 italic text-xs">Sin equipo</span>
                        )}
                      </td>
            ...
            {cargando ? (
              ...
              empleadosFiltrados.map((emp) => (
                <div key={emp.Email} className="p-5 flex flex-col gap-4 active:bg-[var(--bg-hover)] transition-colors">
                  ...
                  <div className="grid grid-cols-2 gap-3 bg-stone-50 p-3 rounded-xl border border-[var(--border-cream)] mb-2">
                  ...
                  <div className="bg-[var(--bg-screen)] p-3 rounded-xl border border-[var(--border-cream)]">
                    <span className="block text-[9px] text-[var(--text-muted)] font-black uppercase tracking-widest mb-1">Equipo Asignado</span>
                    {equipos.find(eq => eq.Email_Empleado === emp.Email) ? (
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-bold text-[var(--text-main)]">{equipos.find(eq => eq.Email_Empleado === emp.Email)?.C_Interno}</span>
                        <span className="text-[10px] text-stone-500 uppercase">{equipos.find(eq => eq.Email_Empleado === emp.Email)?.Marca}</span>
                      </div>
                    ) : (
                      <span className="text-stone-400 italic text-xs">Sin equipo asignado</span>
                    )}
                  </div>
              ...
              <form onSubmit={guardarEmpleado} className="p-6 overflow-y-auto pb-10 sm:pb-6 space-y-6 bg-white">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                  ...
                  <div className="sm:col-span-2 space-y-2 relative">
                    <label className="block text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Asignación de Equipo (C_Interno)</label>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <Laptop size={16} className={busquedaEquipo ? "text-[#71717a]" : "text-slate-500"} />
                      </div>
                      <input
                        type="text"
                        placeholder="Escribe para buscar (Ej: CMP-001)"
                      ...
                      {mostrarSugerencias && busquedaEquipo.length > 0 && (
                        <div className="absolute z-[60] w-full mt-1 bg-white border border-[var(--border-cream)] rounded-xl shadow-2xl max-h-52 overflow-y-auto scrollbar-hide">
                          {sugerenciasEquipos.length > 0 ? (
                                ...
                                >
                                  <div>
                                    <p className={`text-sm font-bold transition-colors ${ocupadoPorOtro ? 'text-stone-400 group-hover:text-red-500' : 'text-[var(--text-main)] group-hover:text-[#71717a]'}`}>
                                      {unidad.C_Interno} {ocupadoPorOtro && '(Asignado)'}
                                    </p>
                                    <p className="text-[10px] text-[var(--text-muted)] uppercase">
                                      {unidad.Marca} {unidad.Modelo} {unidad.Service_Tag ? `• ST: ${unidad.Service_Tag}` : ''}
                                    </p>
                                  </div>
                            ...
                            <div className="p-4 text-center text-xs text-slate-600 italic">No hay resultados para "{busquedaEquipo}"</div>
                          )}
                        </div>
                      )}
                    </div>
