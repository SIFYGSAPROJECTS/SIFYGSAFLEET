...
import { Save, Plus, Trash2, Wallet, CalendarRange } from 'lucide-react';
import GastosMenu from '../GastosMenu';

interface CajaChicaRecord {
  Id_CajaChica?: number;
  Fecha: string;
  No_Factura: string;
  Concepto: string;
...
export default function CajaChicaClient({ userEmail, isAdmin }: { userEmail: string, isAdmin: boolean }) {
  const [registros, setRegistros] = useState<CajaChicaRecord[]>([]);
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/gastos/caja-chica?email=${encodeURIComponent(userEmail)}&semana=${semana}&anio=${anio}`);
      if (res.ok) {
  ...
  const createNewRow = (): CajaChicaRecord => ({
  ...
  const handleCellChange = (index: number, field: keyof CajaChicaRecord, value: any) => {
    const newRegs = [...registros];
    newRegs[index] = { ...newRegs[index], [field]: value };
    recalculateBalances(newRegs);
  };

  const recalculateBalances = (regs: CajaChicaRecord[]) => {
    let currentSaldo = 0;
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/gastos/caja-chica', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: userEmail, registros, semana, anio })
      });
      if (res.ok) {
        alert(`Gastos de la Semana ${semana} guardados exitosamente`);
        fetchRecords();
      } else {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-stone-500 text-sm mt-1">
              Captura tus gastos e ingresos directamente en la tabla.
            </p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-red-50 text-red-900 border-b border-red-200">
              <tr>
                <th className="px-3 py-3 font-semibold text-center border-r border-red-200 w-12">#</th>
                <th className="px-3 py-3 font-semibold border-r border-red-200 w-40">Fecha</th>
                <th className="px-3 py-3 font-semibold border-r border-red-200 w-48">No. Factura</th>
                <th className="px-3 py-3 font-semibold border-r border-red-200 min-w-[300px]">Concepto</th>
                <th className="px-3 py-3 font-semibold border-r border-red-200 w-32 text-right">Cargo</th>
                <th className="px-3 py-3 font-semibold border-r border-red-200 w-32 text-right">Abono</th>
                <th className="px-3 py-3 font-semibold border-r border-red-200 w-32 text-right">Saldo</th>
                <th className="px-3 py-3 font-semibold w-12 text-center"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-stone-200">
              {registros.map((row, index) => (
                <tr key={index} className="hover:bg-red-50/30 focus-within:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-3 py-1.5 border-r border-stone-100">
                    <input
                      ...
                      className="w-full bg-transparent border border-transparent hover:border-stone-200 focus:bg-white focus:border-red-400 rounded px-2 py-1.5 outline-none text-stone-700 text-sm"
                    />
                  </td>
                  <td className="px-3 py-1.5 border-r border-stone-100">
                    <input
                      ...
                      className="w-full bg-transparent border border-transparent hover:border-stone-200 focus:bg-white focus:border-red-400 rounded px-2 py-1.5 outline-none text-stone-700 text-sm font-mono uppercase"
                    />
                  </td>
                  <td className="px-3 py-1.5 border-r border-stone-100">
                    <input
                  ...
                  <td className="px-3 py-1.5 border-r border-stone-100">
                    <input
                      ...
                      className="w-full text-right bg-transparent border border-transparent hover:border-stone-200 focus:bg-white focus:border-red-400 rounded px-2 py-1.5 outline-none text-stone-700 text-sm"
                    />
                  </td>
                  <td className="px-3 py-1.5 border-r border-stone-100">
                    <input
